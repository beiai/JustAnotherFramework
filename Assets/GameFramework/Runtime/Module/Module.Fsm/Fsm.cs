...
namespace GameFramework.Module.Fsm
{
    /// <summary>
    /// 有限状态机。
    /// </summary>
    ...
    internal sealed class Fsm<T> : FsmBase, IReference, IFsm<T> where T : class
    {
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            ...
            {
                throw new Exception(
                    $"FSM '{FullName}' can not start state '{typeof(TState).FullName}' which is not exist.");
            }
        ...
        internal void ChangeState(Type stateType)
        {
            ...
            {
                throw new Exception(
                    $"FSM '{FullName}' can not change state to '{stateType.FullName}' which is not exist.");
            }
