...
namespace GameFramework.Module.Timer
{
    public class TimerManager : IModule
    {
        ...
        /// </summary>
        private readonly SortedDictionaryMultiValue<long, int> _timeId = new SortedDictionaryMultiValue<long, int>();
        ...
        public void Update()
        {
            ...
            while (_timeOutTimerIds.Count > 0)
            {
                ...
                if (timer == null)
                {
                    Log.Error($"获取Timer失败！ID:{timerId}");
                    continue;
                }
            ...
            Action<long> updateCallBack = null)
        {
            if (time < 0)
            {
                Log.Error($"new once time too small: {time}");
            }
